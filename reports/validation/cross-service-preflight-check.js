...
const checks = [
  ...
  },
  {
    ...
    patterns: ['supplier ID matching `TRACE_SUPPLIER_ID`', 'supplier routes owned by `TRACE_SUPPLIER_ID`', 'route evidence must include warehouse and supplier identifiers', 'not owned by the same `TRACE_SUPPLIER_ID`', 'sourceFingerprint` matching `trace:<TRACE_PRODUCT_ID>', 'mismatched supplier job source fingerprint', 'generator refuses dirty service worktrees', 'deployment rows in the final report that do not match the manifest-hashed deployment evidence artifact', 'clean Warehouse, Catalog, and Suppliers worktrees'],
  },
  {
    service: 'suppliers',
