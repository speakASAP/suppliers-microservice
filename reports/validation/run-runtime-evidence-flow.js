...
function validateDeploymentEvidenceFile(filePath) {
  ...
  assert(isCanonicalIsoUtcTimestamp(deployment?.generatedAt), 'DEPLOYMENT_EVIDENCE_FILE generatedAt must be a canonical UTC ISO timestamp');
...
function printPlan() {
  console.log(JSON.stringify({
    ...
    deploymentEvidenceValidation: 'requires DEPLOYMENT_EVIDENCE_FILE generated by create-deployment-evidence.js or accepted by verify-deployment-evidence.js with canonical UTC generatedAt, warehouse/catalog/suppliers commit SHA matching current repo HEAD, deploy command, completed health evidence, and 401/403 protected endpoint evidence',
