...
function runSelfTest() {
  ...
  const services = Object.fromEntries(['warehouse', 'catalog', 'suppliers'].map((service) => [service, {
    commitSha: spawnSync('git', ['-C', path.join(process.env.CROSS_SERVICE_ROOT || '/home/ssf/Documents/Github', `${service === 'warehouse' ? 'warehouse' : service}-microservice`), 'rev-parse', 'HEAD'], { encoding: 'utf8' }).stdout.trim(),
  ...
  writeJson(deploymentFile, { services });
  ...
  runNodeJson(['reports/validation/run-runtime-evidence-flow.js', '--manifest-self-test']);
