...
function render(rows, preflight) {
  ...
  return [
    ...
    '- Suppliers import job belongs to TRACE_SUPPLIER_ID, validates the Catalog product ID, preserves Warehouse authority, records owner-approved mutation, reports positive applied updates, and has a source fingerprint matching the approved trace product and supplier warehouse IDs;',
...
function assertSelfTestContent(markdown) {
  const required = [
    ...
    'source fingerprint matching the approved trace product and supplier warehouse IDs',
