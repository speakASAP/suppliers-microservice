...
| Warehouse logistics returns local and supplier replenishment and dropship route options. | Route type list and route legs proving local warehouse-to-customer fulfillment plus supplier replenishment through Alfares or direct supplier dropship to customer, positive `available` values, `canReserveFromWarehouse=true`, and supplier routes owned by `TRACE_SUPPLIER_ID`. | pending-runtime |
