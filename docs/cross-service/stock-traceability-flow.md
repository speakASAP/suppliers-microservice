...
| Physical warehouse, supplier/dropship warehouse, quantity, reserved, available, movements, reservations, origin metadata, logistics route plans | Warehouse | Warehouse is the only stock authority. |
...
5. Warehouse applies the reconciliation only to supplier/dropship warehouses, preserves movement/reconciliation evidence, and remains stock authority.
...
7. Warehouse product logistics returns local fulfillment, supplier replenishment, and supplier dropship route options from the same stock-origin data.
8. Catalog availability/projection reads Warehouse availability and batch logistics, then forwards stock-origin rows and `availability.logistics` with source `warehouse`.
...
10. Storefront/channel/order consumers can distinguish own physical stock from supplier/dropship virtual stock and see the Warehouse-owned logistics route while still reserving/fulfilling stock through Warehouse.
...
| dropship | Supplier ships directly or via a virtual dropship location. | Order flow must route fulfillment through supplier/dropship process while Warehouse remains availability authority. |
| mixed | A product has both own and supplier/dropship rows. | Allocation can prefer own stock first, then supplier stock according to Warehouse priority and order rules. |
...
- The expected Warehouse availability response carries local and supplier/dropship rows with origin metadata.
- The expected Catalog projection preserves Warehouse-origin rows under `availability.warehouses[]`, preserves Warehouse-owned route options under `availability.logistics`, and keeps `stockQuantity` equal to Warehouse `totalAvailable`.
...
- Create or use approved own and supplier/dropship Warehouse locations.
- Run an approved Suppliers reconciliation for supplier stock.
- Verify Catalog availability/projection returns both local and supplier-origin rows plus `availability.logistics`.
