...
Only after owner approval and deployment evidence are ready, continue with `RUN_APPROVED_RUNTIME_SMOKE=true`, `OWNER_APPROVAL=explicit`, and `SMOKE_ALLOW_MUTATION=true`. The same command with `--config-only` validates approved-smoke prerequisites, requires deployment evidence generated by `create-deployment-evidence-template.js` with `generatedFromCurrentHeads: true` and the completion-verifier reminder, and checks deployment commit SHAs against the current remote repository HEADs. In real execution, the runner also validates owner approval, mutation allowance, cleanup evidence, and deployment evidence before the read-only fixture phase, so incomplete or stale deployment evidence cannot reach the approved import step. The runner writes fixture/smoke JSON, generates the final report, and verifies it.
...
Create the deployment evidence skeleton from current service commit SHAs, then replace the TODO health/protected-endpoint fields after deployment. Regenerate this file after any Warehouse, Catalog, or Suppliers commit; stale deployment evidence is rejected by the guarded runner and bundle verifier:
