...
The synthetic check produced one Catalog product ID and SKU, one idempotency-derived supplier import externalReference, one own stock row, one linked supplier warehouse stock row, one dropship supplier stock row, and one non-reservable supplier diagnostic row without Warehouse-owned supplier linkage. The final projection preserved all origin rows under Warehouse-sourced availability, preserved Warehouse-owned logistics routes and route legs under availability.logistics, kept FlipFlop stockQuantity equal to traceable reservable route availability while raw Warehouse totals stayed under availability, and the Catalog coverage read model classified the product as covered mixed_stock with local, supplier, dropship, and diagnostic supplier availability totals intact.
